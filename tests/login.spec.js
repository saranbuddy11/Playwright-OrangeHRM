...
test.describe("OrangeHRM Login Tests", () => {
    ...
    test("Verify login fails with invalid password", { tag: "@regression" }, async ({ loginPage }) => {
        await loginPage.goto();
        await loginPage.login(USERS.admin.username, "wrong123");
        await loginPage.verifyError("Invalid credentials");
    });

    test("Verify login fails with invalid username", { tag: "@regression" }, async ({ loginPage }) => {
        await loginPage.goto();
        await loginPage.login("WrongUser", USERS.admin.password);
        await loginPage.verifyError("Invalid credentials");
    });

    test("Verify validation message for empty username", { tag: "@regression" }, async ({ loginPage }) => {
        await loginPage.goto();
        await loginPage.login("", USERS.admin.password);
        await loginPage.verifyRequiredMessage("Required");
    });

    test("Verify validation message for empty password", { tag: "@regression" }, async ({ loginPage }) => {
        await loginPage.goto();
        await loginPage.login(USERS.admin.username, "");
        await loginPage.verifyRequiredMessage("Required");
    });

    test("Verify validation messages for empty username and password", { tag: "@regression" }, async ({ loginPage }) => {
        await loginPage.goto();
